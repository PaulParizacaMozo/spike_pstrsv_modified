...
void spike_pstrsv_gpu(const char uplo, int m, double *a, MKL_INT *ia, MKL_INT *ja, double *b, double *x, int nthreads)
{
	// Preprocessing & solving
	spike_pstrsv_preproc(a, ia, ja, m, nthreads, uplo);
	spike_pstrsv_solve_gpu(uplo, m, b, x, nthreads);
}
...
void spike_pstrsv_solve_gpu(const char uplo, int m, double *b, double *x, int nthreads)
{
    int th_id;
    const char transa = 'N';
    const char diag = 'N';        
    ...
    #pragma omp parallel private(th_id) shared(alpha, beta, m, b, x, transa, diag, matdescra, nthreads)
    {
        ...
        int start_R = starts_R[th_id];
        int end_R = ends_R[th_id];
        int start_interval = start_intervals[th_id]; 
        int end_interval = end_intervals[th_id]; 
        char hasDependence = hasDependences[th_id];
        char hasReflection = hasReflections[th_id];
        char isOptimized = isOptimizeds[th_id];
        ...
        if(uplo == 'U' || uplo == 'u')
        {
            ...
            {   
                if(hasReflection || isOptimized)
                {
                    // Parallelize only the loop for __udsol
                    rowCount = k - start_interval;

                    #pragma omp target teams distribute parallel for \
                        map(to: x[partition_start + start_interval:k]) \
                        map(to: b[partition_start + start_interval:k]) \
                        map(to: T2, IT2[start_D1], JT2) \
                        map(from: x[partition_start + start_interval:k])
                    for (int i = 0; i < rowCount; i++) {
                        __udsol(1, &x[partition_start + start_interval + i], &b[partition_start + start_interval + i], T2, &IT2[start_D1], JT2);
                    }
                }

                #pragma omp barrier

                // Continue with sequential computations
                produceReducedY_PSTRSV3(reducedX, x);
                #pragma omp single
                {
                    __usol2(numRowsReducedSystem, reducedX, T, IT, JT);
                ...
                {
                    #pragma omp target teams distribute parallel for \
                        map(to: x[partition_start:partition_end]) \
                        map(from: b[partition_start:partition_end]) \
                        map(to: D[ID[start_R]], JD[ID[start_R]], ID[start_R])
                    for (int i = 0; i < k; i++) {
                        mkl_dcsrmv(&transa, &k, &m, &alpha, matdescra, &D[ID[start_R]], &JD[ID[start_R]], &ID[start_R], &ID[start_R + 1], x, &beta, &b[partition_start + i]);
                    }
                }
                ...
                if(hasReflection || isOptimized)
                {
                    ...
                    {
                        #pragma omp target teams distribute parallel for \
                            map(to: x[partition_start]) \
                            map(to: b[partition_start]) \
                            map(to: T2, IT2[start_D2], JT2) \
                            map(from: x[partition_start])
                        for (int i = 0; i < rowCount; i++) {
                            __udsol(1, &x[partition_start + i], &b[partition_start + i], T2, &IT2[start_D2], JT2);
                        }
                    }
                } 
                else
                {
                    #pragma omp target teams distribute parallel for \
                        map(to: x[partition_start]) \
                        map(to: b[partition_start]) \
                        map(to: D[ID[start_D]], JD[ID[start_D]], ID[start_D]) \
                        map(from: x[partition_start])
                    for (int i = 0; i < k; i++) {
                        __udsol(1, &x[partition_start + i], &b[partition_start + i], D, &ID[start_D], JD);
                    }
                }
            }
            else
...
void _spike_pstrsv_solve_gpu(const char uplo, int m, double *b, double *x, int nthreads)
{
    /*
        Parallel Sparse Triangular System Solver using partial DS Factorization to solve.
        Specifically, it does not compute the whole S matrix, but instead only a small 
        ...
        if(uplo == 'U' || uplo == 'u')
        {
            ...
            {    
                if(hasReflection || isOptimized)
                {
                    ...
                    #pragma omp target teams distribute parallel for map(to: x[partition_start + start_interval : partition_start + start_interval + rowCount], \
                                                                       b[partition_start + start_interval : partition_start + start_interval + rowCount]) \
                                                                   map(from: x[partition_start + start_interval : partition_start + start_interval + rowCount])
                    for (int i = 0; i < rowCount; i++) {
                        __udsol(rowCount, &x[partition_start + start_interval], &b[partition_start + start_interval], T2, &IT2[start_D1], JT2);
                    }
                ...
                {
                    temp_timer[th_id] = omp_get_wtime();
                    #pragma omp target teams distribute parallel for map(to: reducedX[0:numRowsReducedSystem]) map(from: x[0:numRowsReducedSystem])
                    for (int i = 0; i < numRowsReducedSystem; i++) {
                        __usol2(numRowsReducedSystem, reducedX, T, IT, JT);
                    }
                    reduced_solver_timer_pstrsv3 += omp_get_wtime() - temp_timer[th_id];
                }
                ...
                if(hasDependence) // whether the thread has dependent elements (sparse R matrix)
                {
                    // Compute b = b - R * x (if hasReflection -> R = R_old + D3)
                    temp_timer[th_id] = omp_get_wtime();
                    #pragma omp target teams distribute parallel for map(to: D[ID[start_R]], JD[ID[start_R]], ID[start_R : start_R + 1], x[partition_start : partition_start + rowCount]) \
                                                                   map(from: b[partition_start : partition_start + rowCount])
                    for (int i = 0; i < rowCount; i++) {
                        mkl_dcsrmv(&transa, &k, &m, &alpha, matdescra, &D[ID[start_R]], &JD[ID[start_R]], &ID[start_R], &ID[start_R + 1], x, &beta, &b[partition_start]);
                    }
                    subtract_Rx_timer[th_id] += omp_get_wtime() - temp_timer[th_id];
                ...
                if(hasReflection || isOptimized)
                {
                    ...
                    if(rowCount > 0)
                    {
                        #pragma omp target teams distribute parallel for map(to: x[partition_start], b[partition_start]) \
                                                                       map(from: x[partition_start : partition_start + rowCount])
                        for (int i = 0; i < rowCount; i++) {
                            __udsol(rowCount, &x[partition_start], &b[partition_start], T2, &IT2[start_D2], JT2);
                        }
                    }
                ...
                else // no intersection means no need to split the D matrix
                {
                    // Solve D * x = b
                    temp_timer[th_id] = omp_get_wtime();
                    #pragma omp target teams distribute parallel for map(to: D[ID[start_D]], JD[ID[start_D]]) map(from: x[partition_start : partition_start + rowCount])
                    for (int i = 0; i < rowCount; i++) {
                        __udsol(k, &x[partition_start], &b[partition_start], D, &ID[start_D], JD);
                    }
                    solveD_timer[th_id] += omp_get_wtime() - temp_timer[th_id];
